import { ArrowLeftIcon, ArrowRightFromLine } from "lucide-react"
     ...
     <nav className="headerNav text-white pl-4 w-full z-10 flex justify-between items-center">
        <div className="flex items-center">
            <div className="logoSection">
                <div className={`logo ${showSideBar ? "" : "collapsed"}`}>
                    <img src={logo[showSideBar ? 0 : 1]} alt="logo" onClick={() => showSideBar ? "" : setShowSideBar(!showSideBar)} />
                </div>
                <div className={`minimize ${showSideBar ? "" : "collapsed"}`} onClick={() => setShowSideBar(!showSideBar)}>
                    <div className="bg-black text-white p-1 rounded-md">
                        {showSideBar ? <ArrowLeftIcon /> : <ArrowRightFromLine />}
                    </div>
                </div>
            </div>
        </div>
